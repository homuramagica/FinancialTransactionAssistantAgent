...
- FEED는 최신 변화 탐지용으로 기본 확인하되, 핵심 사실/결론은 고신뢰 원출처로 재검증한다.
...
- 데이터 소스는 `yfinance + FEED + 고신뢰 웹 검색`으로 사용한다.
- 기업분석 작성 전 반드시 `기업 검색(회사/티커 중심)`과 `섹터 검색(산업/정책/수급 중심)`을 각각 수행한다.
- 기업 이슈와 섹터 이슈는 분리 기록 후 교차 검증해 본문/결론에 통합한다.
- 관련성이 높다고 판단되면 `python3 scripts/world_memory_cli.py list --days 21 --format md` 또는 `--days 30`을 먼저 조회해 현재 시장 맥락을 내부적으로 파악한다.
- 다만 월드 메모리와 직접 관련이 낮은 개별 기업/니치 업종 분석에서는 별도 섹션을 강제하지 않고, 필요한 경우에만 본문에 자연스럽게 통합한다.
- 기업분석 요청에서도 FEED를 기본적으로 사용한다.
- FEED는 최신 헤드라인 탐지 레이어로 쓰고, 본문 반영 전 고신뢰 웹 검색/공식 자료로 반드시 재확인한다.
...
4. 중기 맥락 확인(조건부)
   - 기업 분석에 실제 영향을 줄 만한 거시/정책/지정학 배경이 보이면 `world_issue_log`를 조회해 현재 진행 중인 중기 스토리를 확인한다.
   - 월드 메모리는 배경 렌즈일 뿐이며, 본문 근거는 반드시 최신 기업 검색/섹터 검색/공식 자료로 다시 채운다.
5. 기업 최신 이슈 검색
...
7. 최신 촉매 통합 검증
   ...
   - FEED 5종은 기본 호출해 최신 변화를 탐지하고, 중요 이슈는 고신뢰 웹 검색/공식 자료로 재검증한다.
8. 분석
...
- 사용자가 "속보/실시간"을 함께 요구하면 FEED 비중을 높이고 더 최신 항목에 가중치를 둔다.
