...
- 데이터 소스는 기본적으로 `yfinance + 고신뢰 웹 검색`으로 고정한다.
...
- 섹터 분석은 FEED를 기본 호출하지 않는다.
- 사용자가 `최신/실시간/속보`를 명시적으로 요청할 때만 FEED 5종을 호출한다.
...
- 사용자가 "속보/실시간"을 함께 말하면 FEED 5종을 추가 호출한다.
