...
- 결과를 `빠른 판단` 또는 `증권사형 장문 보고서`로 출력한다.
...
| 모드 | 트리거 | 기본 산출물 |
| --- | --- | --- |
| 빠른 판단 | 빠른 섹터 질문 | 1페이지 스냅샷 |
| 증권사형 장문 보고서 | `섹터 리포트/PDF/증권사 형식/애널리스트 형식` 요청 | 표지형 1페이지 + 본문 + 결론 + 컴플라이언스 |

`증권사형 장문 보고서` 모드에서는 `references/korean_broker_pdf_style.md`를 우선 적용한다.
...
| 산업 심층 분석 | "반도체 산업 구조 분석해줘" | 증권사형 장문 보고서 |
| 증권사형 섹터 노트 | "증권사 섹터 보고서처럼 써줘" | 증권사형 장문 보고서 |
