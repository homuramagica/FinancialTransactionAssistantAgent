...
- **파이썬은 데이터 정리 중심, 문장 생성 최소화**를 원칙으로 한다.
  ...
  - 빠른 판단 영역은 `레짐 점수 + 신호 + 규칙 적중` 형태의 데이터 블록만 출력한다.
...
- 결과물은 기본적으로 **두 겹 구조**로 출력한다.
  - **빠른 판단**: 의사결정용 20~40줄 요약
  - **심층 분석**: 근거/표/시나리오/체크리스트
- 사용자가 `뉴스 지면 확대`, `뉴스레터 스타일`, `산문 브리핑`, `블룸버그 스타일`을 요청하면,
  심층 분석의 뉴스 파트를 **산문형 뉴스레터 모드**로 확장한다.
  - 권장 문단 수: **8~14문단**
  - 문단당 권장 길이: **3~5문장**
...
- **기준/낙관/비관** 3가지 시나리오를 제시하고, 각 시나리오의 트리거를 불릿으로 적는다.
