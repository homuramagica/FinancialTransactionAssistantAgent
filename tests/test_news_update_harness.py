...
class NewsUpdateHarnessTests(unittest.TestCase):
    def test_fetch_article_with_harness_defaults_to_chrome_devtools(self) -> None:
        ...
        with mock.patch(
            ...
            return_value=_completed(success),
        ) as mock_run:
    ...
    def test_fetch_article_with_harness_falls_back_to_chrome_after_empty_firefox_capture(self) -> None:
        ...
        chrome_success = {
            ...
            "text": "본문 " * 100,
            "html": "<html><article>Recovered article</article></html>",
            "error": None,
            "paywall": False,
        }
        ...
        with mock.patch(
            "scripts.news_update_harness.subprocess.run",
            side_effect=[
                _completed(firefox_failure),
                _completed(chrome_diagnose),
                _completed(chrome_success),
            ],
        ) as mock_run:
            result = harness.fetch_article_with_harness(
                "https://www.bloomberg.com/news/articles/example",
        ...
        self.assertTrue(result["success"])
        self.assertEqual(result["fallback_from_browser"], "firefox-visible")
        self.assertEqual(
    ...
    def test_fetch_article_with_harness_falls_back_to_chrome_after_firefox_launch_error(self) -> None:
        ...
        with mock.patch(
            "scripts.news_update_harness.subprocess.run",
            side_effect=[
                _completed(firefox_failure, returncode=1),
                _completed(chrome_diagnose),
                _completed(chrome_success),
            ],
        ) as mock_run:
    ...
    def test_validate_article_content_rejects_nbsp_without_blank_lines(self) -> None:
        issues = harness.validate_article_content(
            "26-04-10 20-08 은행주, 최악의 1분기 뒤 실적이 반전의 시험대에 섰습니다.md",
            INVALID_NBSP_SPACING_ARTICLE,
        )
    ...
    def test_fetch_article_with_harness_reports_subprocess_timeout(self) -> None:
        with mock.patch(
            "scripts.news_update_harness.subprocess.run",
            side_effect=subprocess.TimeoutExpired(cmd=["python3"], timeout=31),
        ) as mock_run:
            result = harness.fetch_article_with_harness(
                "https://www.wsj.com/articles/example",
                browser="firefox-visible",
        ...
        self.assertFalse(result["success"])
        self.assertIn("45초 제한", result["error"])
        self.assertEqual(result["harness_attempts"], 1)
    ...
    def test_fetch_batch_with_harness_short_circuits_on_preflight_failure(self) -> None:
        diagnose = {
            ...
            "availability": {
                "ok": False,
                "detail": "Firefox 앱을 찾지 못했습니다. checked: /Applications/Firefox.app",
            },
        }
        ...
        self.assertFalse(report["preflight_degraded"])
        self.assertEqual(report["requested"], 2)
    ...
    def test_fetch_batch_with_harness_continues_on_soft_chrome_preflight_failure(self) -> None:
        ...
        with mock.patch.object(harness, "_run_fetch_diagnose", return_value=diagnose), \
             ...
             mock.patch.object(
                 ...
                 side_effect=[
                     {"success": True, "url": "https://example.com/a"},
                     {"success": True, "url": "https://example.com/b"},
                 ],
             ) as mock_fetch:
            report = harness.fetch_batch_with_harness(
                [
        ...
        self.assertTrue(report["ok"])
        self.assertFalse(report["skipped_due_to_preflight"])
        self.assertTrue(report["preflight_degraded"])
    ...
    def test_fetch_batch_with_harness_reuses_chrome_browser_until_cleanup(self) -> None:
        ...
        for call in mock_fetch.call_args_list:
            self.assertFalse(call.kwargs["close_after"])

    def test_fetch_batch_with_harness_reuses_chrome_visible_browser_until_cleanup(self) -> None:
        with mock.patch.object(
            harness,
            "_run_fetch_diagnose",
    ...
    def test_fetch_batch_with_harness_reuses_firefox_visible_browser_until_cleanup(self) -> None:
        with mock.patch.object(
            harness,
            "_run_fetch_diagnose",
            ...
            harness,
            "_run_browser_cleanup",
            return_value={"ok": True, "browser": "firefox-visible", "closed": True},
        ) as mock_cleanup:
            report = harness.fetch_batch_with_harness(
                [
                    "https://example.com/a",
        ...
        self.assertTrue(report["ok"])
        self.assertTrue(report["batch_browser_reused"])
        self.assertEqual(report["cleanup"]["browser"], "firefox-visible")
        self.assertEqual(mock_fetch.call_count, 2)
        self.assertEqual(mock_cleanup.call_count, 1)
        for call in mock_fetch.call_args_list:
            self.assertFalse(call.kwargs["close_after"])

    def test_fetch_batch_with_harness_disables_chrome_fallback_by_default(self) -> None:
        with mock.patch.object(
            harness,
            "fetch_article_with_harness",
            side_effect=[
    ...
    def test_fetch_batch_main_command_can_opt_in_chrome_fallback(self) -> None:
        ...
        self.assertEqual(rc, 0)
        self.assertTrue(mock_fetch.call_args.kwargs["allow_chrome_fallback"])
