...
- 사용자가 `나스닥 옵션 분석`(NDX 옵션체인/NDX 옵션 리포트)을 요청하면 `SKILLs/NasdaqOptionAnalysis/SKILL.md`를 참조한다.
  - 이 스킬은 Nasdaq 100 원지수 옵션을 분석하며 yfinance 심볼은 `^NDX`만 사용한다.
...
## 보고서 한국어 문체 공통 원칙
- 시장/섹터/거시/뉴스/실적/기업/포트폴리오/옵션 등 **모든 보고서형 산출물**에는 `SKILLs/ReportWritingStyle.md`를 공통 적용한다.
- 보고서 본문은 한국어로 먼저 사고하고 한국어 문장으로 작성한다. 영어 원문 표현을 조사만 붙여 끼워 넣는 방식은 금지한다.
- 영어 원어는 회사명, 제품명, 티커, 공식 문서명, URL, 코드, API, 데이터 컬럼명, 또는 번역 시 의미가 흐려지는 전문용어의 첫 등장에만 제한적으로 남긴다.
- 전문용어는 가능하면 `한국어 표현(원어)` 순서로 첫 등장에 병기하고, 이후에는 한국어 표현을 우선 사용한다.
- 제목, 표 머리글, 섹션명도 가능한 한 한국어로 쓴다. `Executive Summary`, `Deep-Dive`, `Flash Layer`, `Risk Radar`, `Bull/Base/Bear` 같은 표현은 템플릿 검증에 꼭 필요한 경우가 아니면 `핵심 요약`, `심층 분석`, `빠른 판단`, `위험 점검`, `낙관/기준/비관`으로 바꾼다.
- 최종 제출 전 불필요하게 남은 영어 명사구를 점검하고, 고유명사/공식명/티커/URL/코드/필수 전문용어가 아닌 표현은 한국어로 고친다.
