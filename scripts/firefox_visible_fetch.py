...
- 사이트별 접속 간격은 기본 20-40초 사이 임의의 시간을 지킵니다.
"""
...
def _copy_view_source_html_and_close_tabs(*, close_article: bool = True) -> str:
    _clear_clipboard()
    lines = [
    ...
    if close_article:
        lines.extend([_command_key_code_line("w"), "    delay 0.2"])
    script = _ui_script(lines)
    completed = _run_applescript(script)
    if completed.returncode != 0:
        detail = (completed.stderr or completed.stdout or "").strip()
...
def _close_current_firefox_tab() -> None:
    script = _ui_script([_command_key_code_line("w"), "    delay 0.2"])
    completed = _run_applescript(script)
    if completed.returncode != 0:
        detail = (completed.stderr or completed.stdout or "").strip()
        raise RuntimeError(f"Firefox 탭 닫기에 실패했습니다. {detail}".strip())


def _reload_current_firefox_tab() -> None:
    script = _ui_script(
        [
            _command_key_code_line("r"),
            f"    delay {_PAGE_SETTLE_SECONDS}",
        ]
    )
    completed = _run_applescript(script)
    if completed.returncode != 0:
        detail = (completed.stderr or completed.stdout or "").strip()
        raise RuntimeError(f"Firefox 새로고침에 실패했습니다. {detail}".strip())
...
def _capture_firefox_content(*, close_article: bool = True, settle: bool = True) -> tuple[str, str]:
    if settle:
        time.sleep(max(0.0, _PAGE_SETTLE_SECONDS))
    rendered_text = _copy_visible_page_text()
    html_text = _copy_view_source_html_and_close_tabs(close_article=close_article)
    return rendered_text, html_text


def _build_result_from_captured_content(
    *,
    canonical_url: str,
    rendered_text: str,
    html_text: str,
    ...
    if _detect_bot_block(title, extracted_text, html_snippet):
        result = _default_failure(canonical_url, "봇 제한 페이지가 반환되었습니다.")
        result.update(
            {
                "title": title,
                "text": extracted_text,
...
def _should_try_bloomberg_reload(
    ...
    *,
    ...
    if reload_used:
        return False
    if _site_access_key(url) != "bloomberg":
        return False
    ...
    if _contains_login_prompt(text[:2500], html_text[:2500]):
        return True
    return bool(text.strip()) and len(text.strip()) < _BLOOMBERG_SHORT_TEXT_THRESHOLD
...
def fetch_article(
    ...
    *,
    ...
    try:
        with _browser_session_lock(lock_timeout=lock_timeout):
            ...
            try:
                ...
                article_tab_open = True
                is_bloomberg = _site_access_key(canonical_url) == "bloomberg"
                rendered_text, html_text = _capture_firefox_content(
                    close_article=not is_bloomberg,
                )
                article_tab_open = is_bloomberg
                result = _build_result_from_captured_content(
                ...
                elif article_tab_open:
                    _close_current_firefox_tab()
                    article_tab_open = False
            finally:
                if article_tab_open:
                    try:
    ...
    return result or _default_failure(canonical_url, "Firefox 수집 결과가 비어 있습니다.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        with news_update_session_lock(reason="firefox_visible_fetch"):
