...
- **거시경제 분석/전망 보고서도 FEED를 기본적으로 사용하지 않는다.**
  - 거시 보고서 기본 소스: `world_issue_log + yfinance + 웹 검색(고신뢰 출처) + 공식 기관 자료`
  - 보고서 작성 전 `python3 scripts/world_memory_cli.py list --days 30 --format md`를 먼저 실행해 최근 누적 이슈를 확인한다.
  - `world_issue_log`는 성장/물가/고용/정책/금융여건/지정학의 **중기 맥락을 잡는 내부 컨텍스트**로 사용하고, 부족한 부분만 고신뢰 웹 검색과 1차 자료로 보강한다.
